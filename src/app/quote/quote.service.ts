...
export class QuoteService {
  ...
  // })(this, function () {
  //   'use strict';
  QUOTE_DATA_BY_SYMBOL: { [name: string]: Quote } = {
    MSFT: {
      Name: 'Microsoft Corp',
      Symbol: 'MSFT',
      LastPrice: 72.28,
  ...
  public getQuote(
    symbol: string
  ): Observable<{ Status: string; data?: Quote; Message?: string }> {
    return from(this._getQuote(symbol));
  }
  private async _getQuote(
    symbol: string
  ): Promise<{ Status: string; data: Quote }> {
    symbol = String(symbol).toUpperCase();
    let data = this.QUOTE_DATA_BY_SYMBOL[symbol];
    let response: any;
    if (data) {
      response = {
        Status: 'SUCCESS',
